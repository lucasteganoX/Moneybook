...
Log_Purchase() { # synopsis: Log_Purchase Account_Name Purchase_Cost Purchase_Message # note: write a purchase entry to the log file. It needs the name of the account the purchase was made over, its cost and optionally a message to describe the purchase 
		...
		# Log file checks
		if [[ ! -a "$Log_File_Path" ]] ; then echo "Couldn't log purchase, log file \`${Log_File}\` doesn't exist. Status 7" > /dev/stderr ; return 7 ; fi
		if [[ ! -f "$Log_File_Path" ]] ; then echo "Couldn't log purchase, log file \`${Log_Path}\` is not a regular file. Status 8" > /dev/stderr ; return 8 ; fi
		if [[ ! -w "$Log_File_Path" ]] ; then echo "Couldn't log purchase, log file \`${Log_File}\` is not writable. Status 9" > /dev/stderr ; return 9 ; fi
